...
c mass of the pair
      call bookupeqbins('mass-t-tbar',10d0,0d0,1000d0)
c pt of top
      call bookupeqbins('t-pt',10d0,0d0,1000d0)
c pt asymmetry
      call bookupeqbins('t-tbar,pt',10d0,0d0,1000d0)
c rapidity
      call bookupeqbins('t-y',0.2d0,-3d0,3d0)
c rapidity asymmetry
      call bookupeqbins('t-tbar-y',0.2d0,-3d0,3d0)
c (Non b) jet pt
      call bookupeqbins('jet-pt',2d0,0d0,200d0)
c$$$c (Non b) jet y
c$$$      do icut=20,100,20
c$$$         write(cut,'(i3)') icut
c$$$         call bookupeqbins('yjet,pt>'//cut,0.2d0,-4d0,4d0)
c$$$      enddo
c$$$c (Non b) jet y-yttbar
c$$$      do icut=20,100,20
c$$$         write(cut,'(i3)') icut
c$$$         diag=diag+1
c$$$         binsize(diag)=0.2
c$$$         call bookupeqbins('yjet-yttbar,pt>'//cut,
c$$$     1        0.2d0,-4d0,4d0)
c$$$      enddo
c$$$c Lepton+
c$$$c Lepton energy in top rest frame 
c$$$      call bookupeqbins('eem',1d0,0d0,100d0)
c$$$c Lepton cos theta in top rest frame 
c$$$      call bookupeqbins('cos thdec',0.1d0,-1d0,1d0)
c$$$c Lepton azimuth in top rest frame. The origin is the top
c$$$c transverse direction
c$$$      call bookupeqbins('phi dec',pi/20,-pi,pi)
...
c mass of the pair
     ...
     1     -ppairttbar(1)**2-ppairttbar(2)**2-ppairttbar(3)**2)
      ...
      call filld('mass-t-tbar',mttbar,dsig)
c pt of top
      call filld('t-pt',ptt,dsig)
c pt asymmetry
      call filld('t-tbar,pt',ptt,dsig)
      call filld('t-tbar,pt',pttbar,-dsig)
c rapidity
      call filld('t-y',yt,dsig)
c rapidity asymmetry
      call filld('t-tbar-y',yt,dsig)
      call filld('t-tbar-y',ytbar,-dsig)
c (Non b) jet pt
      call filld('jet-pt',ptjet,dsig)
c$$$c (Non b) jet y
c$$$      do icut=20,100,20
c$$$         diag=diag+1
c$$$         if(ptjet.gt.icut) then
c$$$            call filld(diag,yjet,dsig)
c$$$         endif
c$$$      enddo
c$$$c (Non b) jet y-yttbar
c$$$      do icut=20,100,20
c$$$         diag=diag+1
c$$$         if(ptjet.gt.icut) then
c$$$            call filld(diag,yjet-yttbar,dsig)
c$$$         endif
c$$$      enddo
c$$$c leptons may not be there, in case we are called by the NLO
c$$$c section
c$$$      if(iep.eq.0.or.iem.eq.0) return         
c$$$c Lepton+
c$$$c Compute lepton variables in top rest frame
c$$$      call decvariables(phep(1,iep),phep(1,it),eep,cthep,phiep)
c$$$      call decvariables(phep(1,iem),phep(1,itbar),eem,cthem,phiem)
c$$$c Lepton energy in top rest frame 
c$$$      diag=diag+1
c$$$      call filld(diag,eep,dsig)
c$$$c Lepton cos theta in top rest frame 
c$$$      diag=diag+1
c$$$      call filld(diag,cthep*cthem,dsig)
c$$$c Lepton azimuth in top rest frame. The origin is the top
c$$$c transverse direction
c$$$      diag=diag+1
c$$$      dphi=(phiep-phiem)-nint((phiep-phiem)/(2*pi))*2*pi
c$$$      call filld(diag,dphi,dsig)
      end
